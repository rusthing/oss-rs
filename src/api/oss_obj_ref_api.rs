...
pub async fn add(
    ...
    let result = oss_obj_ref_svc::add(to, None).await?;
...
///
/// ## 请求体
/// * `OssObjRefModifyTo` - 包含待修改记录信息的结构体
///
/// ## 请求头
/// * `USER_ID_HEADER_NAME` - 当前用户ID，必需项，类型为u64
///
...
pub async fn modify(
    ...
    let result = oss_obj_ref_svc::modify(to, None).await?;
    Ok(HttpResponse::Ok().json(result))
}
...
pub async fn save(
    ...
    let result = oss_obj_ref_svc::save(to, None).await?;
    Ok(HttpResponse::Ok().json(result))
}
...
pub async fn del(
    ...
    Ok(HttpResponse::Ok().json(oss_obj_ref_svc::del(id, current_user_id, None).await?))
}
...
/// * 当根据ID找不到对应记录时，返回相应的错误信息
#[utoipa::path(
    path = "/oss/obj-ref/get-by-id",
    params(
        ("id", description = "记录的唯一标识符，类型为u64")
    ),
    responses(
        (status = OK, body = Ro<OssObjRefVo>)
    )
)]
#[get("/get-by-id")]
pub async fn get_by_id(
    ...
    let ro = oss_obj_ref_svc::get_by_id(id, None).await?;
