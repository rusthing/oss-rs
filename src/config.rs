...
pub fn init_config(path: Option<String>, port: Option<u16>) {
    // 读取配置文件内容到字符串
    let content = if path.is_some() {
        ...
        let config_file_name = format!("{}.yml", env.app_file_name);
        let config_file_path = env.app_dir.join(&config_file_name);
        Some(fs::read_to_string(config_file_path).expect("读取配置文件失败，可能是配置文件不存在"))
    };
    ...
    CONFIG.set(config).expect("无法设置配置信息");
}
