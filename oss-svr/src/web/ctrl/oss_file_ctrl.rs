...
//             let range = range
//                 .to_str()
//                 .map_err(|_| CtrlError::from(validator::ValidationError::new("无效的Range")))?;
...
// fn parse_obj_id(obj_id: &String) -> Result<(String, Option<String>), CtrlError> {
...
//         (obj_id, Some(ext))
//     } else {
//         let message = format!("路径<id>格式不正确: {}", obj_id);
//         return Err(CtrlError::from(validator::ValidationError::new(Box::leak(
